 ...
 */
export class ProductsRequestBuilder<T extends DeSerializers = DefaultDeSerializers> extends RequestBuilder<Products<T>, T> {
  /**
  ...
  getByKey(productId: DeserializedType<T, 'Edm.Int32'>): GetByKeyRequestBuilder<Products<T>, T> {
    return new GetByKeyRequestBuilder<Products<T>, T>(this.entityApi, { ProductID: productId });
  }
  ...
  delete(productIdOrEntity: any): DeleteRequestBuilder<Products<T>, T> {
    return new DeleteRequestBuilder<Products<T>, T>(this.entityApi, productIdOrEntity instanceof Products ? productIdOrEntity : { ProductID: productIdOrEntity! });
  }
}
